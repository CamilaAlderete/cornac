...
def split_base_adapt_chunks_test(data, base_frac=BASE_FRAC, adapt_total_frac=ADAPT_TOTAL_FRAC, n_adapt_chunks=4):
    n = len(data)
...
def main():
    ...
    log_path = os.path.join(RESULTS_DIR, f"online_ibpr_mejorado_chunk_sensitivity_{timestamp}.txt")
    csv_path = os.path.join(RESULTS_DIR, f"online_ibpr_mejorado_chunk_sensitivity_{timestamp}.csv")
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        ...
        with redirect_stdout(tee):
            ...
            for n_chunks in CHUNK_OPTIONS:
                print("=" * 80)
                print(f"CHUNK SENSITIVITY EXPERIMENT | n_chunks={n_chunks}")
                print("=" * 80)

                base_raw, adapt_chunks_all, test_raw_all = split_base_adapt_chunks_test(
                    data, BASE_FRAC, ADAPT_TOTAL_FRAC, n_chunks
                )
                ...
                ibpr = IBPR(
                    ...
                    verbose=True, name=f"IBPR base | n_chunks={n_chunks}"
                )
                ...
                print_result_block(f"IBPR baseline | n_chunks={n_chunks}", ibpr_train_time, ibpr_test_time, ibpr_metrics)
                ...
                online = OnlineIBPRMejorado(
                    ...
                    name=f"{PARTIAL_CONFIG['label']} | n_chunks={n_chunks}",
                )
                ...
                for stage_idx, chunk_raw in enumerate(adapt_chunks, start=1):
                    ...
                    print_result_block(
                        f"{PARTIAL_CONFIG['label']} | n_chunks={n_chunks} | stage={stage_idx}/{len(adapt_chunks)} | chunk_size={len(chunk_raw):,}",
            ...
            print("=" * 80)
            print("CHUNK SENSITIVITY EXPERIMENT SAVED")
            print("=" * 80)
            print(csv_path)
