...
PROTOCOL_VERSION = "final_h1_h3_v2_20260911"
BASE_SPLIT_RULE = (
...
CHUNK_SPLIT_RULE = (
    "4 approximately equal RAW future chunks by cumulative row targets; "
    "each internal boundary moves forward to the end of its timestamp group; "
    "warm-start filtering is applied only after raw boundaries are frozen"
)
...
SUMMARY_FIELDS = [
    ...
    "all_online_v_exact_equal_base",
    "max_online_v_abs_diff_base",
    "h1_support_classification",
    "h2_support_classification",
...
def split_future_raw_chunks_strict(future_rows, n_chunks=N_STREAM_CHUNKS):
    n = len(future_rows)
    if n < n_chunks:
        raise ValueError("Future stream demasiado pequeño para 4 chunks.")
    ...
    for k in range(1, n_chunks):
        ...
        if target >= n:
            raise ValueError("No se pudo crear un límite interno de chunk válido.")

        effective = target
        boundary_ts = future_rows[target - 1][3]
        while effective < n and future_rows[effective][3] == boundary_ts:
            effective += 1

        if effective >= n:
            raise ValueError(
                "Un empate de timestamp en el límite de chunk consumiría todo el resto "
                "del stream. El protocolo exige abortar."
            )
        if effective <= boundaries[-1]:
            raise RuntimeError("Límite de chunk no creciente.")
    ...
    boundaries.append(n)
    chunks = [
        list(future_rows[boundaries[idx] : boundaries[idx + 1]])
...
def warm_start_filter(base_rows, raw_chunks):
    ...
    for raw_chunk in raw_chunks:
        ...
        for row in raw_chunk:
            ...
            if known_u and known_i:
                ...
                excluded_item += 1
        if not known:
            raise ValueError("Un chunk quedó vacío después del filtrado warm-start.")
        known_chunks.append(known)

    n_future_raw = sum(len(chunk) for chunk in raw_chunks)
    n_future_warm = sum(len(chunk) for chunk in known_chunks)
...
def prepare_protocol_data():
    ...
    raw_chunks, boundaries, adjustments = split_future_raw_chunks_strict(
        split["future_rows"]
    )
    warm = warm_start_filter(split["base_rows"], raw_chunks)

    # Dataset-level pair uniqueness/leakage guard.
    base_pairs = rows_pairs(split["base_rows"])
    observed = set(base_pairs)
    for idx, chunk in enumerate(warm["known_chunks"], start=1):
    ...
    return {
        ...
        "raw_chunk_boundaries": boundaries,
        "raw_chunk_adjustments": adjustments,
        **warm,
    }
...
def print_plan(data, protocol_hash):
    ...
    print("Global chronological chunks (raw boundaries frozen before warm filtering):")
    for idx, (raw, warm) in enumerate(
        zip(data["raw_chunks"], data["known_chunks"]), start=1
    ):
        print(
            f"  chunk_{idx}: raw={len(raw):,} | warm={len(warm):,} "
            f"({len(warm)/len(raw):.2%}) | users={len({r[0] for r in warm}):,} "
            f"| items={len({r[1] for r in warm}):,} | "
            f"raw_ts={ts_min(raw)}..{ts_max(raw)} | "
            f"warm_ts={ts_min(warm)}..{ts_max(warm)}"
        )

    print("  Internal boundary adjustments:")
    for adj in data["raw_chunk_adjustments"]:
        print(
    ...
    print("H2 timing                       : partial_fit_recent vs IBPR.fit only")
...
def run_seed_trial(seed, data, protocol_hash):
    ...
    for step_idx in range(3):
        ...
        eval_rows = list(data["known_chunks"][step_idx + 1])

        if not update_rows or not eval_rows:
            raise RuntimeError("Update/eval chunk vacío durante ejecución.")

        # Strict temporal and pair-level leakage guards.
        if ts_max(update_rows) >= ts_min(eval_rows):
            raise RuntimeError(
                f"step {eval_point}: update/eval no son estrictamente temporales."
            )

        eval_pairs = rows_pairs(eval_rows)
        ...
        # Common evaluation/history dataset for all three branches.
        start_eval_build = time.perf_counter()
        ...
        eval_build_time = time.perf_counter() - start_eval_build
        ...
        # Same evaluation sets for all branches.
        ...
        row = {
            ...
            "eval_min_timestamp": ts_min(eval_rows),
            "eval_max_timestamp": ts_max(eval_rows),
            "eval_dataset_build_time_s": eval_build_time,
...
def classify_h1(step_rows):
    metric = f"online_minus_stale_NDCG@{TOP_K}"
    all_values = [float(row[metric]) for row in step_rows]
    overall_mean = float(np.mean(all_values))
    positive_count = sum(value > 0 for value in all_values)

    seed_means = []
    for seed in FINAL_SEEDS:
        values = [float(row[metric]) for row in step_rows if int(row["seed"]) == seed]
        if len(values) != 3:
            raise RuntimeError("No hay 3 puntos por seed al clasificar H1.")
        seed_means.append(float(np.mean(values)))

    if overall_mean <= 0:
        return "SIN SOPORTE DIRECCIONAL"
    if all(value > 0 for value in seed_means) and positive_count == 6:
        return "SOPORTE FUERTE"
    if all(value > 0 for value in seed_means) and positive_count >= 5:
        return "SOPORTE CONSISTENTE"
    return "EVIDENCIA MIXTA"


def classify_h2(trial_rows):
    fractions = [float(row["online_full_cost_fraction"]) for row in trial_rows]
    if not all(value < 1.0 for value in fractions):
        return "SIN SOPORTE DIRECCIONAL"
    if all(value <= 0.50 for value in fractions):
        return "SOPORTE FUERTE (>=2x EN AMBAS SEEDS)"
    return "REDUCCION DE COSTE (<2x EN AL MENOS UNA SEED)"
...
def build_summary(step_rows, trial_rows, data, protocol_hash):
    ...
    summary = {
        ...
        "h1_support_classification": classify_h1(step_rows),
        "h2_support_classification": classify_h2(trial_rows),
    }
...
def print_final_summary(summary, trial_rows):
    ...
    print()
    print(f"H1 classification              : {summary['h1_support_classification']}")
    print(f"H2 classification              : {summary['h2_support_classification']}")
    print(
...
def main():
    ...
    if args.plan_only:
        print("PLAN-ONLY complete. No model was trained and no final-result file was written.")
        return
