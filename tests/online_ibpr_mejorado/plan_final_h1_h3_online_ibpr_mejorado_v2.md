# Plan experimental pre-registrado y validado V2 — H1-H3 de OnlineIBPRMejorado
...
Se define primero un corte objetivo:

```python
target_base_end = int(len(all_positive_rows) * 0.60)
```

Para impedir que un mismo timestamp quede dividido entre base y future, el corte efectivo se ajustará únicamente cuando sea necesario:

```text
1. tomar el timestamp de la última fila incluida por el corte objetivo;
2. si existen filas inmediatamente posteriores con el mismo timestamp,
   mover el límite hacia adelante hasta terminar ese grupo;
3. exigir max_timestamp(base) < min_timestamp(future).
```

Por tanto, el base efectivo puede ser mínimamente superior al 60% objetivo. El script deberá registrar:
...
Esta regla preserva todas las filas utilizadas por el corte de desarrollo previo y evita trasladar al stream final filas pertenecientes al mismo instante temporal que el base. El tramo futuro efectivo constituye el stream definitivo H1-H3.
...
El script registrará tiempo de preparación de datasets en columnas separadas.

No se mezclará con el tiempo principal de H2. El tiempo Online primario seguirá siendo exclusivamente `partial_fit_recent(...)` y el tiempo Full primario exclusivamente `IBPR.fit(...)`.
...
El resumen final y/o el log deberán exponer explícitamente los promedios de `Online - Stale` por `eval_point` además de los promedios por seed.
...
Además de las validaciones anteriores, en cada punto se verificará fuga a nivel de par:

```text
(u,i) de eval ∩ (u,i) del historial post-update = vacío
```

Esto es más fuerte que comparar filas completas con timestamp y evita evaluar como futuro un par usuario-ítem ya observado.
...
## 22. Fingerprint, reanudación y atomicidad

Antes de reutilizar resultados, el script calculará un fingerprint reproducible que incluya como mínimo:
...
O014
FINAL_SEEDS
reglas de split/chunks/warm-start
métricas
políticas de seeds
hash del dataset implícito ordenado
hash del script
hash de los cores/wrappers IBPR y OnlineIBPRMejorado
```
...
Si un timestamp existente contiene un fingerprint diferente o incompleto:

```text
ABORTAR RESUME
USAR UN TIMESTAMP NUEVO
```
...
Una seed se considerará completa únicamente si existen exactamente:
...
update_chunk = {1,2,3}
eval_chunk = {2,3,4}
1 trial agregado con n_eval_points = 3
```
...
## 30. Enmiendas metodológicas incorporadas en V2

Esta V2 congela antes de observar resultados H1-H3 cuatro endurecimientos respecto del borrador anterior:

```text
1. corte 60/40 estricto por timestamp, moviendo hacia adelante sólo empates del timestamp límite;
2. fingerprint de protocolo/datos/código obligatorio para resume;
3. fuga evaluada también a nivel de par (u,i);
4. resume válido únicamente para seeds estructuralmente completas con eval_chunk={2,3,4}.
```

Estos cambios no modifican R900, O014, las seeds `[777,999]`, la métrica primaria, las tres ramas ni la secuencia prequential. Su propósito es reforzar trazabilidad, temporalidad y reproducibilidad antes de acceder a las métricas definitivas.
