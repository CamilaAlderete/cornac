...
# Frozen R900/O014 implementation fingerprints from the project sources used
# for the final H1-H3 stage.
EXPECTED_IBPR_WRAPPER_SHA256 = "e173018d3cd4de03a2b4e4abea3446f26081be3e96a1d494d3d75c107c9cee8e"
EXPECTED_IBPR_CORE_SHA256 = "10e07672f092ec4995ebadd81975c87a6e3a82cb89e8e25b8ae31bf386a5151c"
EXPECTED_ONLINE_WRAPPER_SHA256 = "4d298f4490324252f9dc5cd61e78bafad0e91e582e7c29f3312c67101bf215da"
EXPECTED_ONLINE_CORE_SHA256 = "867e31e364159f77ddde84e0fbfc46b5799be21467c5fda3de103b8dccd5891f"
...
PROTOCOL_VERSION = "h4_index_reuse_v1_3_final_audited_20260914"
...
STEP_FIELDS = [
    ...
    "full_v_sha256",
    "online_factors_unchanged_by_retrieval",
    "full_factors_unchanged_by_retrieval",
    "base_v_unchanged_by_retrieval",
...
SUMMARY_FIELDS = [
    ...
    "all_base_index_object_same",
    "all_full_rebuilt_index_sha256_unchanged",
    "all_online_factors_unchanged_by_retrieval",
    "all_full_factors_unchanged_by_retrieval",
...
def protocol_payload(data_hash):
    return {
        ...
        "h4_conditions": {
            "online_reused": "query U_online against the one base index built on V_base; GT exhaustive(V_online,U_online)",
            "full_stale": "query U_full against the base index built on V_base; GT exhaustive(V_full,U_full)",
            "full_rebuilt": "query U_full against a faithful rebuilt index on V_full; GT exhaustive(V_full,U_full)",
        },
        "full_retrain_seed_policy": "reset NumPy/Torch to trial seed before each Full fit",
        "online_seed_policy": "Online wrapper internal seed progression from trial seed",
        "latency_repeats": LATENCY_REPEATS,
        "ann_latency_definition": (
            "knn_query + remove -1/out-of-range + dedup + filter seen + take k_eff; "
        ...
        "summary_aggregation": (
            "step CSV reports per seed-point aggregates; summary overall retrieval "
            "metrics pool query-user-condition observations across seeds/points"
        ),
        "retrieval_metrics": [
            "set_recall",
            "position_agreement",
...
def validate_implementation_contracts():
    ...
    environment_mismatches = {
        key: (actual_environment[key], EXPECTED_ENVIRONMENT[key])
        for key in EXPECTED_ENVIRONMENT
        if actual_environment[key] != EXPECTED_ENVIRONMENT[key]
    }
    if environment_mismatches:
        raise RuntimeError(
            "El entorno ya no coincide con el preflight/final congelado: "
            f"{environment_mismatches}"
        )

    frozen_model_fingerprints = {
        "IBPR wrapper": _source_sha256(IBPR),
        "IBPR experiments": _source_sha256(ibpr_core),
        "Online wrapper": _source_sha256(OnlineIBPRMejorado),
        "Online experiments": _source_sha256(online_core),
    }
    expected_model_fingerprints = {
        "IBPR wrapper": EXPECTED_IBPR_WRAPPER_SHA256,
        "IBPR experiments": EXPECTED_IBPR_CORE_SHA256,
        "Online wrapper": EXPECTED_ONLINE_WRAPPER_SHA256,
        "Online experiments": EXPECTED_ONLINE_CORE_SHA256,
    }
    model_fingerprint_mismatches = {
        key: (
            frozen_model_fingerprints[key],
            expected_model_fingerprints[key],
        )
        for key in expected_model_fingerprints
        if frozen_model_fingerprints[key] != expected_model_fingerprints[key]
    }
    if model_fingerprint_mismatches:
        raise RuntimeError(
            "IBPR/OnlineIBPRMejorado ya no coinciden con las implementaciones "
            f"congeladas de H1-H3: {model_fingerprint_mismatches}"
        )
    ...
    print("  Frozen R900/O014 source fingerprints: OK")
    print("  Frozen environment versions: OK")
    print()
...
def print_plan(data, protocol_hash):
    ...
    print("  Environment exact-match       : PASS")
    print("  R900/O014 fingerprints        : PASS")
    print("  ANN source fingerprints       : PASS")
    print()
    print("Threadpool fingerprint after numerical imports:")
...
def run_seed_trial(seed, data, protocol_hash):
    ...
    for step_idx in range(3):
        ...
        # Faithful current Full index: exactly one operational rebuild at this point.
        full_v_before_index = np.asarray(full_model.V).copy()
        full_rebuild_count_before = int(tracker["full_rebuild"])
        ...
        full_rebuild_delta = (
            int(tracker["full_rebuild"]) - full_rebuild_count_before
        )
        if full_rebuild_delta != 1:
            raise RuntimeError(
                f"step {eval_point}: Full rebuild delta={full_rebuild_delta}, "
        ...
        online_u_before_queries = np.asarray(online_model.U).copy()
        online_v_before_queries = np.asarray(online_model.V).copy()
        full_u_before_queries = np.asarray(full_model.U).copy()
        full_v_before_queries = np.asarray(full_model.V).copy()
        base_v_before_queries = np.asarray(base_model.V).copy()
        ...
        online_factors_unchanged_by_retrieval = bool(
            arrays_bitwise_equal(online_model.U, online_u_before_queries)
            and arrays_bitwise_equal(online_model.V, online_v_before_queries)
        )
        full_factors_unchanged_by_retrieval = bool(
            arrays_bitwise_equal(full_model.U, full_u_before_queries)
            and arrays_bitwise_equal(full_model.V, full_v_before_queries)
        )
        base_v_unchanged_by_retrieval = arrays_bitwise_equal(
            base_model.V, base_v_before_queries
        )

        if not online_factors_unchanged_by_retrieval:
            raise RuntimeError(
                f"step {eval_point}: retrieval modificó factores Online."
            )
        if not full_factors_unchanged_by_retrieval:
            raise RuntimeError(
                f"step {eval_point}: retrieval modificó factores Full."
            )
        if not base_v_unchanged_by_retrieval:
            raise RuntimeError(
                f"step {eval_point}: retrieval modificó V_base."
            )
        if not arrays_bitwise_equal(online_model.V, base_v):
            raise RuntimeError(
                f"step {eval_point}: V_online dejó de ser bitwise V_base "
                "después de retrieval."
            )
        ...
        step_row = {
            ...
            "base_v_unchanged_by_retrieval": base_v_unchanged_by_retrieval,
            ...
            "full_operational_index_rebuild_count": full_rebuild_delta,
...
def build_summary(step_rows, query_rows, trial_rows, data, protocol_hash):
    ...
    summary = {
        ...
        "all_base_index_object_same": all(
            (
                bool_from_csv(row["base_index_object_same"])
                if isinstance(row["base_index_object_same"], str)
                else bool(row["base_index_object_same"])
            )
            for row in step_rows
        ),
        "all_full_rebuilt_index_sha256_unchanged": all(
            (
                bool_from_csv(row["full_rebuilt_index_sha256_unchanged"])
                if isinstance(row["full_rebuilt_index_sha256_unchanged"], str)
                else bool(row["full_rebuilt_index_sha256_unchanged"])
            )
            for row in step_rows
        ),
        "all_online_factors_unchanged_by_retrieval": all(
            (
                bool_from_csv(row["online_factors_unchanged_by_retrieval"])
                if isinstance(row["online_factors_unchanged_by_retrieval"], str)
                else bool(row["online_factors_unchanged_by_retrieval"])
            )
            for row in step_rows
        ),
        "all_full_factors_unchanged_by_retrieval": all(
            (
                bool_from_csv(row["full_factors_unchanged_by_retrieval"])
                if isinstance(row["full_factors_unchanged_by_retrieval"], str)
                else bool(row["full_factors_unchanged_by_retrieval"])
            )
            for row in step_rows
        ),
...
def print_summary(summary):
    ...
    print(
        f"Same base ANN object all points : "
        f"{summary['all_base_index_object_same']}"
    )
    print(
        f"Full rebuilt SHA unchanged      : "
        f"{summary['all_full_rebuilt_index_sha256_unchanged']}"
    )
    print(
        f"Online factors query-readonly   : "
        f"{summary['all_online_factors_unchanged_by_retrieval']}"
    )
    print(
        f"Full factors query-readonly     : "
        f"{summary['all_full_factors_unchanged_by_retrieval']}"
    )
    print()
