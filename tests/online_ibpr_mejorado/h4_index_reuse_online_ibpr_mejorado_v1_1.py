...
PROTOCOL_VERSION = "h4_index_reuse_v1_1_hardened_20260914"
...
def seed_complete(seed, step_rows, query_rows, trial_rows):
    ...
    expected_total_queries = sum(EXPECTED_PRIMARY_USERS) * len(CONDITIONS)
    if len(queries) != expected_total_queries:
        return False

    expected_counts = {
        (point, condition): EXPECTED_PRIMARY_USERS[point - 1]
        for point in (1, 2, 3)
